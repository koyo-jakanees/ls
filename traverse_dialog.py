...
class TraverseDialog(QDialog):
    ...
    def __init__(self, log):
        ...
        self.ui.StartPointComboBox.currentIndexChanged.connect(
            self.startpointComboChanged
        )
        ...
        if targets is not None:
            for target in targets:
                item = QListWidgetItem(
                    u"%s (%s:%s)" % (target[0], target[1], target[2])
                )
    ...
    def onRemoveButton(self):
        ...
        """
        self.ui.TargetList.addItem(
            self.ui.OrderList.takeItem(self.ui.OrderList.currentRow())
        )
        if self.ui.OpenRadio.isChecked():
            self.fillOpenTraverseEndPoints()
    ...
    def onCalcButton(self):
        ...
        if self.ui.buttonGroup.checkedId() == -1:
            QMessageBox.warning(
                self, tr("Warning"), tr("Select the type of traverse line!")
            )
            return
            ...
            if targets is not None:
                for target in targets:
                    if i > 0 and target[0] == stations[i - 1][0]:
                        ...
                        self.ui.OpenRadio.isChecked()
                        and i == len(stations) - 1
                        and target[0] == endpoint[0]
                    ):
                        if obs2 is None:
                            obs2 = get_target(target[0], target[1], target[2])
                        if obs1 is not None:
        ...
        if self.ui.OpenRadio.isChecked():
            trav_obs.append(
                [
                    Station(
                        Point(endpoint[0]), PolarObservation(endpoint[0], "station")
                    ),
                    None,
                    None,
                ]
        ...
        if plist is not None:
            ...
            QMessageBox.warning(
                self, tr("Warning"), tr("Traverse line cannot be calculated!")
            )
            self.ui.ResultTextBrowser.append(ResultLog.resultlog_message)
            self.log.write(ResultLog.resultlog_message)
    ...
    def onHelpButton(self):
        ...
        """
        webbrowser.open(
            "http://www.digikom.hu/SurveyingCalculation/usersguide.html#traverse-calculations"
        )
