...
class BatchPlottingDialog(QDialog):
    ...
    def onTempDirButton(self):
        ...
        templatepath = QFileDialog.getExistingDirectory(
            self, tr("Select Directory"), self.templatepath, QFileDialog.ShowDirsOnly
        )
        if templatepath != "":
        ...
        if self.batch_plotting:
            ...
            if selected_polygons is None:
                QMessageBox.warning(
                    ...
                    tr("Warning"),
                    tr(
                        "Select at least one polygon on layer '%s'!"
                        % selected_layer.name()
                    ),
                )
                return
        ...
        if self.batch_plotting:
            ...
            if self.ui.OutputTab.currentIndex() == 0:  # to PDF
                ...
                if self.ui.SingleFileCheckbox.checkState():
                    ...
                    outputFileName, __ = QFileDialog.getSaveFileName(
                        self,
                    ...
                    if (
                        not QDir(outputDir).exists()
                        or not QFileInfo(outputDir).isWritable()
                    ):
                        QMessageBox.warning(
                            self,
                            tr("Unable to write into the directory"),
                            tr(
                ...
                for featureI in range(0, atlas.numFeatures()):
                    ...
                    if not self.ui.SingleFileCheckbox.checkState():
                        ...
                        outputFileName = (
                            QDir(outputDir).filePath(atlas.currentFilename()) + ".pdf"
                        )
