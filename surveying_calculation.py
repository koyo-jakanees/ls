...
class SurveyingCalculation(object):
    ...
    # noinspection PyMethodMayBeStatic def tr(self, message): """Get the translation for a string using Qt
    # translation API. We implement this ourselves since we do not inherit QObject.
    ...
    def create_coordlist(self):
        ...
        ofname, __, __ = QFileDialog.getSaveFileName(
            self.iface.mainWindow(),
        ...
        if QRegExp("coord_").indexIn(QFileInfo(ofname).baseName()):
            ...
            for ext in [".shp", ".shx", ".dbf"]:
                ...
                coord = QgsVectorLayer(
                    ofbase + ".shp", QFileInfo(ofbase).baseName(), "ogr"
                )
                if coord.isValid():
    ...
    def load_fieldbook(self):
        ...
        if get_coordlist() is None:
            QMessageBox.warning(
                ...
                tr("Warning"),
                tr(
                    "No coordinate list is opened, coordinates "
                    "will be lost from the fieldbook"
                ),
            )
        homedir = QSettings().value("SurveyingCalculation/homedir", config.homedir)
        fname, __, __ = QFileDialog.getOpenFileName(
            self.iface.mainWindow(),
        ...
        if fname:
            ...
            if QRegExp("\.are$", Qt.CaseInsensitive).indexIn(fname) == -1:
                ...
                QSettings().setValue(
                    "SurveyingCalculation/homedir", QFileInfo(fname).absolutePath()
                )
                QSettings().sync()
                # start with 'fb_'?
                if QRegExp("fb_").indexIn(QFileInfo(ofname).baseName()):
            ...
            if QRegExp("\.gsi$", Qt.CaseInsensitive).indexIn(fname) > -1:
                ...
                QRegExp("\.job$", Qt.CaseInsensitive).indexIn(fname) > -1
                or QRegExp("\.are$", Qt.CaseInsensitive).indexIn(fname) > -1
            ):
                fb = JobAre(fname)
            ...
            while True:
                ...
                if "station" in r:
                    ...
                    record.setFields(fb_dbf.pendingFields())
    ...
    def polygon_division(self):
        ...
        if (
            al is None
            or al.type() != QgsMapLayer.VectorLayer
            or al.geometryType() != Qgis.Polygon
        ):
            QMessageBox.warning(
                self.iface.mainWindow(),
                tr("Warning"),
    ...
    def settings(self):
        ...
        if result == QDialog.Accepted:
            log_path = QSettings().value(
                "SurveyingCalculation/log_path", config.log_path
            )
    ...
    def about(self):
        ...
        QMessageBox.information(
            ...
            tr("About"),
            tr(
                "Surveying Calculation Plugin\n\n (c) DigiKom Ltd 2014- http://digikom.hu, mail (at) "
                "digikom.hu\nVersion 0.2 "
            ),
        )
