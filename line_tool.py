...
class LineMapTool(QgsMapToolEmitPoint):
    ...
    def canvasPressEvent(self, e):
        ...
        if al is None or al.type() != QgsMapLayer.VectorLayer or al.geometryType() != 2:
            QMessageBox.warning(
    ...
    def divide(self):
        ...
        point1 = QgsPoint(
            float(coord_dlg.ui.StartEast.text()), float(coord_dlg.ui.StartNorth.text())
        )
        point2 = QgsPoint(
            float(coord_dlg.ui.EndEast.text()), float(coord_dlg.ui.EndNorth.text())
        )
        # center of rotation
        point0 = point1
        rotate = True
        ...
        while True:
            ...
            if rotate:  # change line direction
                ...
                dir += atan(b / l)
                point1 = QgsPoint(
                    point0.x() + sin(dir + pi) * 1000.0,
                    point0.y() + cos(dir + pi) * 1000.0,
                )
                point2 = QgsPoint(
                    point0.x() + sin(dir) * 1000.0, point0.y() + cos(dir) * 1000.0
                )
            else:  # offset line
                # perpendicular direction to line
                b = da / l  # approximate offset
                dir += pi / 2.0  # perpendicular dir
                point1 = QgsPoint(point1.x() + sin(dir) * b, point1.y() + cos(dir) * b)
                point2 = QgsPoint(point2.x() + sin(dir) * b, point2.y() + cos(dir) * b)
            i += 1
            if i > max_iteration:
                QMessageBox.warning(
                    self.iface.mainWindow(),
